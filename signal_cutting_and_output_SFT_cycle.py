...
setted_parameter = {
                     'cycle_start_number':int(15),'cycle_interval':int(5),'cycle_end_number':int(24),'time_shift_length':0.001,
                     'sorted_number_location':int(4),'origin_data_path':'origin_data',
                     'cutted_signal_time':0.025, 'sampling_frequency':int(2000000),'remove_offset':int(1),'log_process':int(0),'guiyihua':int(1),
...
def signal_cutting_and_output_SFT_cycle( cycle_start_number,cycle_interval,cycle_end_number,time_shift_length,
                                         ...
                                         remove_offset,log_process,guiyihua,seted_seg_length,selected_highest_requency):
    ...
    for saved_file in saved_file_list:
        ...
        for file_0 in sorted(csv_file_0, key = sort_by_number_in_filename):   #把路径下的文件按照时间顺序排序并依次读取 key = os.path.getctime
            ...
            for i in range(int(cycle_number)):
                ...
                spectrogram_0 = singnal_0.make_spectrogram(seg_length=seted_seg_length) #这里可以调整，调节两个分辨率的制衡
                array_0 = spectrogram_0.plot(high=selected_highest_requency)  #这里可以设置时频图里面显示的最高频率，然后把时频图的数据读取给array_0
                #print(array_0.shape)

                if int(log_process)==int(1):
                    array_0 = np.log10(array_0)

                if int(guiyihua)==int(1):
