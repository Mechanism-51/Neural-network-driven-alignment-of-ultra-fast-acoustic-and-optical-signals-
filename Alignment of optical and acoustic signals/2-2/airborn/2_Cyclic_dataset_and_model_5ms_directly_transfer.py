...
seted_parameter = {'setted_SFT_path':['SFT_log_0_time_shift_0.001_0','SFT_log_0_time_shift_0.001_5','SFT_log_0_time_shift_0.001_10','SFT_log_0_time_shift_0.001_15','SFT_log_0_time_shift_0.001_20'],
                   ...
                   'setted_batch_size':512,'seted_num_epochs':100, 'seted_lr':0.0005,
                   'setted_loss_and_accuracy_file_name':'Accuracy_SFT_log_0_time_shift_0.005_-60_-50_directly_transfer'','seted_sample_number/2':250,
                   'setted_start_cycle_number':-60,'setted_end_cycle_number':-50}
...
for cyclic_number in cyclic_array:
	...
	for file_number,png_list_5ms in enumerate(png_list):     #Process time-lapse and sampling data to establish a dataset
		...
		valid_d1_for_CNN = DataLoader(self_dataset,seted_parameter['seted_sample_number/2'],shuffle=False)
		...
		torch.manual_seed(1)
		hist = caculate(model, valid_d1_for_CNN)
		#print(type(hist))


		print(f'hist[1]',np.max(hist[1]))
		highest_accuracy[highest_accuracy_index] = np.max(hist[1])   #Calculate the highest validation accuracy for each cycle
