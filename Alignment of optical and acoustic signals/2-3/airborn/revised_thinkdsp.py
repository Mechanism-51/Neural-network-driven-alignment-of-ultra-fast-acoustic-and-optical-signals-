...
class _SpectrumParent:
    ...
    def plot(self, high=None, **options):
        ...
        if self.full:
            ...
            #plt.plot(fs, amps, **options)                       #原来不是注释
            return fs, amps                                      #后添加的
        else:
            i = None if high is None else find_index(high, self.fs)
            #plt.plot(self.fs[:i], self.amps[:i], **options)     #原来不是注释
            return  self.fs[:i], self.amps[:i]                   #后开发的程序
...
#This program is a modified version of the program in URL https://github.com/AllenDowney/ThinkDSP.